...
async function swapTokens(terraClient: LCDClient, chainId: string, wallet: Wallet, fromTokens: string, toTokens: string, amount: number) {
    ...
    const transaction =  await wallet.createAndSignTx({ msgs: [swap] }).then(tx => terraClient.tx.broadcast(tx));
      // .then(console.info)
      // .catch((err) => {
      //   if (err.response) {
      ...
      //   }
      // });
    return `https://finder.terra.money/${chainId}/tx/${transaction.txhash}`;
}

async function terraSender(terraClient: LCDClient, chainId: string, wallet: Wallet, toWallet: string, tokenType: string, amount: number) {
  ...
  const transaction = await wallet.createAndSignTx({msgs: [my_tx]}).then(tx => terraClient.tx.broadcast(tx));
  // .then(console.info)
  // .catch((err) => {
  //   if (err.response) {
  //     console.error(err.response.data);
...
const {terra, wallet} = createWallet(mnemonic, 'luna', 0.000001);
const chain = "bombay-12";

//swapTokens(terra, wallet, 'luna', 'usd', 250);

terraSender(terra, chain, wallet, "terra1axmdadm204mcyh4vqstgjl59us9tc3cah78s3j", 'luna', 100).then(t => console.log(t));
